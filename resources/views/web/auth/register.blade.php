...
<body class="min-h-screen flex items-center justify-center p-4">
    ...
    <div class="login-card">
        <h1 class="text-2xl font-bold text-gray-900 text-center mb-8">
            وانطلق لاكتشاف أجمل الوجهات السياحية والعروض المميزة
        </h1>
        ...
        <form id="registerForm" action="{{ route('register') }}" method="POST">
            ...
            <div class="flex items-center justify-end gap-2 mb-6">
                <span class="text-sm text-gray-600">الموافقة على الشروط والأحكام</span>
    ...
    <script>
        ...
        document.getElementById('registerForm').addEventListener('submit', function (e) {
            ...
            if (!csrfToken) {
                Swal.fire({
                    title: 'خطأ',
                    text: 'حدث خطأ في التحقق من الأمان. يرجى تحديث الصفحة.',
                    icon: 'error'
                });
                return;
            }
            ...
            fetch(form.action, {
                ...
                .then(async response => {
                    ...
                    if (response.ok && data.success) {
                        Swal.fire({
                            ...
                            // Add cache-busting timestamp to prevent showing stale logged-out state
                            const redirectUrl = new URL(data.redirect, window.location.origin);
                            redirectUrl.searchParams.set('v', new Date().getTime());
                ...
                .catch(error => {
                    ...
                    if (error.errors) {
                        ...
                        genErr.innerText = error.message || 'حدث خطأ غير متوقع. يرجى المحاولة مرة أخرى.';
