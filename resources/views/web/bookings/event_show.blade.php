<!DOCTYPE html>
<html lang="ar" dir="rtl">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>سافر | تفاصيل تذكرة الفعالية #{{ $ticket->ticket_reference }}</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        body {
            font-family: Cairo, sans-serif;
            background-color: #F8F9FB;
        }

        .booking-card {
            border-radius: 32px;
            background: #fff;
            box-shadow: 0 10px 40px rgba(0, 0, 0, 0.03);
        }

        .status-badge {
            padding: 0.5rem 1.5rem;
            border-radius: 100px;
            font-weight: 700;
            font-size: 0.875rem;
        }

        .status-pending { background-color: #FEF3C7; color: #92400E; }
        .status-approved, .status-active { background-color: #D1FAE5; color: #065F46; }
        .status-cancelled { background-color: #FEE2E2; color: #991B1B; }
        .status-completed { background-color: #DBEAFE; color: #1E40AF; }

        .detail-item {
            display: flex;
            align-items: center;
            gap: 1rem;
            padding: 1rem;
            background: #F8F9FB;
            border-radius: 20px;
        }

        .detail-icon {
            width: 44px;
            height: 44px;
            border-radius: 14px;
            display: flex;
            align-items: center;
            justify-content: center;
            background: #fff;
            color: #2563eb;
            box-shadow: 0 4px 10px rgba(0, 0, 0, 0.02);
            flex-shrink: 0;
        }
    </style>
</head>

<body>
    <!-- Alerts Wrapper -->
    <div class="fixed top-6 left-1/2 -translate-x-1/2 z-[9999] w-[90%] max-w-md space-y-3">
        @if(session('success'))
            <div class="bg-green-100 border border-green-200 text-green-800 px-6 py-4 rounded-2xl shadow-xl flex items-center gap-3 animate-bounce">
                <i class="fas fa-check-circle"></i>
                <p class="font-bold">{{ session('success') }}</p>
            </div>
        @endif
        @if(session('error'))
            <div class="bg-red-100 border border-red-200 text-red-800 px-6 py-4 rounded-2xl shadow-xl flex items-center gap-3">
                <i class="fas fa-exclamation-circle"></i>
                <p class="font-bold">{{ session('error') }}</p>
            </div>
        @endif
        @if(session('info'))
            <div class="bg-blue-100 border border-blue-200 text-blue-800 px-6 py-4 rounded-2xl shadow-xl flex items-center gap-3">
                <i class="fas fa-info-circle"></i>
                <p class="font-bold">{{ session('info') }}</p>
            </div>
        @endif
    </div>

    @include('partials.navbar')

    <main class="py-12 px-4 md:px-8 max-w-5xl mx-auto min-h-screen">
        <!-- Breadcrumb & Back -->
        <div class="flex items-center justify-between mb-8">
            <a href="{{ route('web.bookings.index', ['tab' => 'events']) }}" class="flex items-center gap-2 text-gray-500 hover:text-blue-600 transition font-bold">
                <i class="fa-solid fa-arrow-right"></i>
                <span>العودة لحجوزاتي</span>
            </a>
            <div class="flex items-center gap-2 text-sm text-gray-400">
                <a href="{{ route('web.home') }}">الرئيسية</a>
                <i class="fa-solid fa-chevron-left text-[10px]"></i>
                <a href="{{ route('web.bookings.index') }}">حجوزاتي</a>
                <i class="fa-solid fa-chevron-left text-[10px]"></i>
                <span class="text-gray-900 font-bold">تفاصيل تذكرة الفعالية</span>
            </div>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <!-- Main Content -->
            <div class="lg:col-span-2 space-y-8">
                <!-- Header Info -->
                <div class="booking-card p-8 text-right">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                        <div>
                            <span class="status-badge status-{{ $ticket->status }}">
                                @switch($ticket->status)
                                    @case('pending') قيد المراجعة @break
                                    @case('confirmed') @case('approved') @case('active') تم التأكيد @break
                                    @case('cancelled') ملغى @break
                                    @case('completed') مكتمل @break
                                    @default {{ $ticket->status }}
                                @endswitch
                            </span>
                        </div>
                        <div class="md:text-left">
                            <p class="text-gray-400 text-sm mb-1 uppercase tracking-wider font-bold">إجمالي المبلغ</p>
                            <p class="text-4xl font-bold text-blue-600">{{ number_format($ticket->total_price) }} <span class="text-lg">ج.م</span></p>
                        </div>
                    </div>

                    @php
                        $event = $ticket->event;
                    @endphp

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="detail-item">
                            <div class="detail-icon"><i class="fa-solid fa-calendar-day"></i></div>
                            <div>
                                <p class="text-xs text-gray-400 mb-0.5">تاريخ الفعالية</p>
                                <p class="font-bold text-gray-900">{{ \Carbon\Carbon::parse($event->event_date)->translatedFormat('d F Y') }}</p>
                            </div>
                        </div>
                        <div class="detail-item">
                            <div class="detail-icon"><i class="fa-solid fa-clock"></i></div>
                            <div>
                                <p class="text-xs text-gray-400 mb-0.5">الوقت</p>
                                <p class="font-bold text-gray-900">{{ \Carbon\Carbon::parse($event->event_date)->translatedFormat('h:i A') }}</p>
                            </div>
                        </div>
                        <div class="detail-item md:col-span-2">
                            <div class="detail-icon"><i class="fa-solid fa-location-dot"></i></div>
                            <div>
                                <p class="text-xs text-gray-400 mb-0.5">الموقع</p>
                                <p class="font-bold text-gray-900">{{ $event->location_ar }}</p>
                            </div>
                        </div>
                        <div class="detail-item">
                            <div class="detail-icon"><i class="fa-solid fa-ticket"></i></div>
                            <div>
                                <p class="text-xs text-gray-400 mb-0.5">عدد التذاكر</p>
                                <p class="font-bold text-gray-900">{{ $ticket->tickets_count }} تذكرة</p>
                            </div>
                        </div>
                         <div class="detail-item">
                            <div class="detail-icon"><i class="fa-solid fa-circle-check"></i></div>
                            <div>
                                <p class="text-xs text-gray-400 mb-0.5">الفئة</p>
                                <p class="font-bold text-gray-900">عام</p>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Event Details Section -->
                <div class="booking-card p-8 text-right">
                    <h3 class="text-xl font-bold text-gray-900 mb-6">حول الفعالية</h3>
                    <div class="flex flex-col md:flex-row gap-6">
                        @php
                            $images = $event->activity_images;
                            $firstImage = is_array($images) && count($images) > 0 ? $images[0] : null;
                        @endphp
                        @if($firstImage)
                        <div class="w-full md:w-48 h-32 rounded-2xl overflow-hidden shrink-0">
                            <img src="{{ asset('storage/' . $firstImage) }}" class="w-full h-full object-cover" alt="{{ $event->name_ar }}">
                        </div>
                        @endif
                        <div class="flex-1">
                            <h4 class="font-bold text-lg text-gray-900 mb-2">{{ $event->name_ar }}</h4>
                            <p class="text-sm text-gray-500 leading-relaxed">{{ Str::limit($event->description_ar, 200) }}</p>
                        </div>
                    </div>
                </div>

                @if($ticket->notes)
                <!-- Notes -->
                <div class="booking-card p-8 text-right">
                    <h3 class="text-xl font-bold text-gray-900 mb-4">ملاحظات إضافية</h3>
                    <div class="bg-gray-50 p-6 rounded-2xl text-gray-600 leading-relaxed italic">
                        " {{ $ticket->notes }} "
                    </div>
                </div>
                @endif
            </div>

            <!-- Sidebar -->
            <div class="lg:col-span-1 space-y-8">
                <!-- Status & Payment -->
                <div class="booking-card p-6 text-right">
                    <h3 class="font-bold text-gray-900 mb-4">ملخص الدفع</h3>
                    <div class="space-y-3">
                        <div class="flex items-center justify-between text-sm">
                            <span class="font-bold text-gray-900">{{ number_format($ticket->total_price) }} ج.م</span>
                            <span class="text-gray-400">الإجمالي</span>
                        </div>
                        @if($ticket->status === 'confirmed' || $ticket->status === 'approved' || $ticket->status === 'active')
                        <div class="pt-3 border-t border-dashed flex items-center justify-between">
                            <span class="font-bold text-xl text-blue-600">{{ number_format($ticket->total_price) }} ج.م</span>
                            <span class="text-gray-900 font-bold">المتبقي</span>
                        </div>
                        <a href="{{ route('web.payments.initiate', ['type' => 'event_ticket', 'id' => $ticket->id]) }}" 
                           class="block w-full mt-6 bg-blue-600 text-white py-4 rounded-2xl font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-100 text-center">
                            اكمل الدفع الآن
                        </a>
                        @else
                        <div class="bg-gray-50 p-4 rounded-xl text-center text-sm text-gray-500 mt-4 italic">
                            سيتم تفعيل خيار الدفع فور تأكيد الحجز من قبل الإدارة
                        </div>
                        @endif
                    </div>
                </div>

                <!-- Type Badge -->
                <div class="booking-card p-6 text-right">
                    <div class="flex items-center justify-end gap-3">
                        <div>
                            <p class="font-bold text-gray-900">تذكرة فعاليات وأنشطة</p>
                            <p class="text-xs text-gray-400">نوع الحجز</p>
                        </div>
                        <div class="w-12 h-12 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center shrink-0">
                            <i class="fa-solid fa-ticket text-xl"></i>
                        </div>
                    </div>
                </div>

                <!-- Support -->
                <div class="bg-blue-600 rounded-[32px] p-6 text-white text-right relative overflow-hidden">
                    <div class="relative z-10">
                        <h3 class="font-bold text-lg mb-2">هل لديك استفسار؟</h3>
                        <p class="text-blue-100 text-sm mb-6">يسعدنا خدمتك والرد على جميع استفساراتك حول الفعالية</p>
                        <a href="{{ route('web.contact') }}" class="inline-flex items-center gap-2 bg-white text-blue-600 px-6 py-2.5 rounded-xl font-bold hover:bg-blue-50 transition">
                            <span>تواصل معنا</span>
                            <i class="fa-solid fa-headset"></i>
                        </a>
                    </div>
                    <i class="fa-solid fa-headset absolute -left-4 -bottom-4 text-8xl text-blue-500/20"></i>
                </div>
            </div>
        </div>
    </main>

    @include('partials.footer')
</body>

</html>
