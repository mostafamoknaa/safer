...
@section('title', __('hotel.payments.create_title'))
@section('page-title', __('hotel.payments.create_heading'))
@section('page-subtitle', __('hotel.payments.create_subheading', ['reference' => $booking->booking_reference]))
    ...
    <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur">
        ...
        <div class="mb-6 rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-500">{{ __('hotel.payments.form.booking') }}</p>
                    <p class="text-lg font-bold text-slate-900">{{ $booking->booking_reference }}</p>
                </div>
                <div class="text-left">
                    <p class="text-sm text-slate-500">{{ __('hotel.bookings.table.total_price') }}</p>
                    <p class="text-lg font-bold text-slate-900">{{ number_format($booking->total_price, 2) }} {{ __('hotel.payments.currency') }}</p>
                    <p class="text-sm text-slate-500">{{ __('hotel.payments.remaining_amount') }}</p>
                    <p class="text-lg font-bold text-emerald-600">{{ number_format($booking->remaining_amount, 2) }} {{ __('hotel.payments.currency') }}</p>
                </div>
            </div>
        </div>

        <form method="POST" action="{{ route('hotel.payments.store') }}" class="space-y-6">
            @csrf
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div class="grid gap-2">
                    <label for="transaction_id" class="text-sm font-medium text-slate-600">
                        {{ __('hotel.payments.form.transaction_id') }}
                    </label>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
                ...
                <button type="submit"
                    ...
                    {{ __('hotel.payments.messages.created') }}
                </button>
            </div>
        </form>
    </div>
