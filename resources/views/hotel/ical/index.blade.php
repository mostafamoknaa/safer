    ...
    <div class="grid gap-6">
        ...
        <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('hotel.hotel-rooms.index', ['hotel_id' => $hotelRoom->hotel_id]) }}"
        ...
        <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur">
            ...
            <p class="text-sm text-slate-600 mb-4">استخدم هذا الرابط لمزامنة حجوزاتك مع Airbnb أو Booking.com أو أي منصة أخرى</p>
        ...
        <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur">
            ...
            {{-- Add New URL Form --}}
            <form method="POST" action="{{ route('hotel.ical.store', $hotelRoom) }}" class="mb-6">
                @csrf
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @foreach($hotelRoom->icalUrls as $icalUrl)
                                <tr>
                                    ...
                                    <td class="px-4 py-4 text-center">
                                        <form method="POST" action="{{ route('hotel.ical.destroy', [$hotelRoom, $icalUrl]) }}" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
