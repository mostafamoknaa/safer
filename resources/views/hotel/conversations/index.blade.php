...
@section('title', __('hotel.conversations.page_title'))
@section('page-title', __('hotel.conversations.heading'))
@section('page-subtitle', __('hotel.conversations.subheading'))
...
@section('content')
    <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-lg font-semibold text-slate-900">{{ __('hotel.conversations.heading') }}</h2>
                <p class="text-sm text-slate-500">{{ __('hotel.conversations.subheading') }}</p>
            </div>
            @if($unreadCount > 0)
                <div class="inline-flex items-center gap-2 rounded-xl bg-rose-500/10 px-4 py-2 text-sm font-semibold text-rose-600">
                    <i class="fas fa-envelope"></i>
                    <span>{{ $unreadCount }} {{ __('hotel.conversations.unread_messages') }}</span>
                </div>
            @endif
        </div>
        ...
        <div class="mt-6 grid gap-4 sm:grid-cols-2">
            <form method="GET" action="{{ route('hotel.conversations.index') }}" class="flex items-center gap-2">
                <input type="text" name="search" value="{{ request('search') }}"
                       placeholder="{{ __('hotel.conversations.search_placeholder') }}"
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('hotel.conversations.index', ['status' => 'open']) }}"
                   @class([
                    ...
                    {{ __('hotel.conversations.open') }}
                </a>
                <a href="{{ route('hotel.conversations.index', ['status' => 'closed']) }}"
                   @class([
                       'inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold transition',
                    ...
                    {{ __('hotel.conversations.closed') }}
                </a>
                @if(request('status'))
                    <a href="{{ route('hotel.conversations.index') }}"
                       class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-100">
                        {{ __('hotel.conversations.all') }}
                    </a>
                @endif
            </div>
        </div>
        ...
        <div class="mt-6 overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 text-right text-sm">
                <thead class="bg-slate-50 text-xs font-medium uppercase tracking-wider text-slate-500">
                    <tr>
                        <th class="px-4 py-3">{{ __('hotel.conversations.table.user') }}</th>
                        <th class="px-4 py-3">{{ __('hotel.conversations.table.last_message') }}</th>
                        <th class="px-4 py-3">{{ __('hotel.conversations.table.last_message_at') }}</th>
                        <th class="px-4 py-3 text-center">{{ __('hotel.conversations.table.status') }}</th>
                        <th class="px-4 py-3 text-center">{{ __('hotel.conversations.table.actions') }}</th>
                    </tr>
                </thead>
                            ...
                            <td class="px-4 py-4 text-center">
                                @if ($conversation->status === 'open')
                                    <span class="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-600">
                                        ...
                                        {{ __('hotel.conversations.badges.open') }}
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500">
                                        ...
                                        {{ __('hotel.conversations.badges.closed') }}
                                    </span>
                                @endif
                            </td>
                            <td class="px-4 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('hotel.conversations.show', $conversation) }}"
                                       class="inline-flex items-center gap-1 rounded-lg bg-indigo-500/10 px-3 py-1.5 text-xs font-semibold text-indigo-600 hover:bg-indigo-500/20">
                                        ...
                                        {{ __('hotel.conversations.actions.open') }}
                                    </a>
                                    @if($conversation->status === 'open')
                                        <form method="POST" action="{{ route('hotel.conversations.close', $conversation) }}">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
                                                ...
                                                <i class="fas fa-archive"></i>
                                                {{ __('hotel.conversations.actions.close') }}
                                            </button>
                                        </form>
                                    @endif
                                </div>
