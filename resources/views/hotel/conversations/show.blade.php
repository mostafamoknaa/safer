...
@section('title', __('hotel.conversations.show_title'))
@section('page-title', __('hotel.conversations.show_heading'))
@section('page-subtitle', __('hotel.conversations.show_subheading', ['user' => $conversation->user->name]))
...
@section('content')
    <div class="grid gap-6">
        ...
        <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @else
                        <form method="POST" action="{{ route('hotel.conversations.reopen', $conversation) }}">
                            @csrf
                            @method('PATCH')
                            <button type="submit"
                                ...
                                {{ __('hotel.conversations.actions.reopen') }}
                            </button>
                        </form>
                    @endif
                    <a href="{{ route('hotel.conversations.index') }}"
                       class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-100">
                        ...
                        {{ __('hotel.conversations.back_to_list') }}
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur">
            <div id="messagesContainer" class="space-y-4 max-h-[600px] overflow-y-auto mb-6 pb-6">
                ...
                @empty
                    <div class="text-center py-12 text-slate-500">
                        ...
                        <p>{{ __('hotel.conversations.no_messages') }}</p>
                    </div>
                @endforelse
            </div>
            ...
            {{-- Send Message Form --}}
            @if($conversation->status === 'open')
                <form method="POST" action="{{ route('hotel.conversations.send-message', $conversation) }}" 
                    ...
                    <div class="grid gap-4">
                        <div class="grid gap-2">
                            <label for="message" class="text-sm font-medium text-slate-600">
                                {{ __('hotel.conversations.form.message') }}
                            </label>
                            <textarea id="message" name="message" rows="3"
                                      ...
                                      placeholder="{{ __('hotel.conversations.form.message_placeholder') }}"></textarea>
                            @error('message')
                        ...
                        <div class="grid gap-2">
                            <label for="file" class="text-sm font-medium text-slate-600">
                                {{ __('hotel.conversations.form.file') }}
                            </label>
                            <input id="file" name="file" type="file"
                                   class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200">
                            <p class="text-xs text-slate-400">{{ __('hotel.conversations.form.file_hint') }}</p>
                            @error('file')
                                <p class="text-xs text-rose-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="flex items-center justify-end gap-3">
                            <button type="submit"
                                ...
                                {{ __('hotel.conversations.actions.send') }}
                            </button>
                        </div>
                    </div>
                </form>
            @else
                <div class="border-t border-slate-200 pt-6 mt-6 text-center text-slate-500">
                    <p>{{ __('hotel.conversations.conversation_closed') }}</p>
                </div>
            @endif
        </div>
    </div>
