...
<head>
    ...
    <title>{{ __('hotel.auth.page_title') }}</title>
    ...
    <div class="flex min-h-screen items-center justify-center px-4 py-10">
        <div class="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl ring-1 ring-slate-200/70">
            <div class="text-center">
                ...
                <h1 class="mt-4 text-2xl font-semibold text-slate-900">{{ __('hotel.auth.heading') }}</h1>
                <p class="mt-2 text-sm text-slate-500">{{ __('hotel.auth.subheading') }}</p>
            </div>
            ...
            <form method="POST" action="{{ route('hotel.login.store') }}" class="mt-8 space-y-6">
                @csrf
                <div class="space-y-2">
                    <label for="email" class="block text-sm font-medium text-slate-700">
                        {{ __('hotel.auth.email_label') }}
                    </label>
                    <input
                        type="email"
                        name="email"
                        ...
                        placeholder="{{ __('hotel.auth.email_placeholder') }}"
                        value="{{ old('email') }}"
                    >
                </div>
                <div class="space-y-2">
                    <label for="password" class="block text-sm font-medium text-slate-700">
                        {{ __('hotel.auth.password_label') }}
                    </label>
                    <input
                        type="password"
                        name="password"
                        ...
                        class="block w-full rounded-lg border border-slate-300 bg-slate-50 px-4 py-2 text-sm text-slate-900 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200"
                        placeholder="{{ __('hotel.auth.password_placeholder') }}"
                    >
                </div>
                <div class="flex items-center justify-between">
                    <label for="remember" class="flex items-center gap-2 text-sm text-slate-600">
                        ...
                        >
                        <span>{{ __('hotel.auth.remember') }}</span>
                    </label>
                </div>
                <button
                    type="submit"
