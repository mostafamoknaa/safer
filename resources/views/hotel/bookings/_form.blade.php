...
<div class="grid gap-6">
        ...
        <div class="grid gap-2">
            ...
            <select id="hotel_id" name="hotel_id"
                ...
                @foreach($hotels as $hotel)
                    <option value="{{ $hotel->id }}" @selected(old('hotel_id', optional($booking)->hotel_id ?? '') == $hotel->id)>
        ...
        <select id="room_id" name="room_id"
            ...
            @if(isset($rooms) && $rooms->count() > 0)
                @foreach($rooms as $room)
                    <option value="{{ $room->id }}" @selected(old('room_id', optional($booking)->room_id ?? '') == $room->id)>
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        <div class="grid gap-2">
            ...
            <input id="check_in_date" name="check_in_date" type="date"
                   value="{{ old('check_in_date', optional(optional($booking)->check_in_date)->format('Y-m-d') ?? '') }}"
                   class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                   required>
            @error('check_in_date')
                <p class="text-xs text-rose-600">{{ $message }}</p>
        ...
        <div class="grid gap-2">
            ...
            <input id="check_out_date" name="check_out_date" type="date"
                   value="{{ old('check_out_date', optional(optional($booking)->check_out_date)->format('Y-m-d') ?? '') }}"
                   class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                   required>
    ...
    <div class="grid gap-4 sm:grid-cols-3">
        <div class="grid gap-2">
            ...
            <input id="guests_count" name="guests_count" type="number" min="1"
                   value="{{ old('guests_count', optional($booking)->guests_count ?? '1') }}"
                   class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                   required>
            @error('guests_count')
                <p class="text-xs text-rose-600">{{ $message }}</p>
        ...
        <div class="grid gap-2">
            ...
            <input id="rooms_count" name="rooms_count" type="number" min="1"
                   value="{{ old('rooms_count', optional($booking)->rooms_count ?? '1') }}"
                   class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                   required>
            @error('rooms_count')
                <p class="text-xs text-rose-600">{{ $message }}</p>
        ...
        <div class="grid gap-2">
            ...
            <input id="price_per_night" name="price_per_night" type="number" step="0.01" min="0"
                   value="{{ old('price_per_night', optional($booking)->price_per_night ?? '') }}"
                   class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                   required>
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        <div class="grid gap-2">
            ...
            <select id="status" name="status"
                    class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                    required>
                <option value="pending" @selected(old('status', optional($booking)->status ?? 'pending') == 'pending')>
                    {{ __('hotel.bookings.status.pending') }}
                </option>
                <option value="confirmed" @selected(old('status', optional($booking)->status ?? '') == 'confirmed')>
                    {{ __('hotel.bookings.status.confirmed') }}
                </option>
                <option value="checked_in" @selected(old('status', optional($booking)->status ?? '') == 'checked_in')>
                    {{ __('hotel.bookings.status.checked_in') }}
                </option>
                <option value="checked_out" @selected(old('status', optional($booking)->status ?? '') == 'checked_out')>
                    {{ __('hotel.bookings.status.checked_out') }}
                </option>
                <option value="cancelled" @selected(old('status', optional($booking)->status ?? '') == 'cancelled')>
        ...
        <div class="grid gap-2">
            ...
            <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                <span id="total_price_display">{{ number_format(old('total_price', optional($booking)->total_price ?? 0), 2) }}</span> {{ __('hotel.bookings.currency') }}
            </div>
            <input type="hidden" id="nights_count" name="nights_count" value="{{ old('nights_count', optional($booking)->nights_count ?? 1) }}">
        </div>
    </div>
    ...
    <div class="grid gap-2">
        ...
        <textarea id="notes" name="notes" rows="3"
                  class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200">{{ old('notes', optional($booking)->notes ?? '') }}</textarea>
        @error('notes')
            <p class="text-xs text-rose-600">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="grid gap-2">
        ...
        <textarea id="admin_notes" name="admin_notes" rows="3"
                  class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200">{{ old('admin_notes', optional($booking)->admin_notes ?? '') }}</textarea>
        @error('admin_notes')
            <p class="text-xs text-rose-600">{{ $message }}</p>
        @enderror
    </div>
