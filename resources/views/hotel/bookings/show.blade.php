...
@section('title', __('hotel.bookings.show_title'))
@section('page-title', __('hotel.bookings.show_heading', ['reference' => $booking->booking_reference]))
@section('page-subtitle', __('hotel.bookings.show_subheading'))
...
@section('content')
    <div class="grid gap-6">
        ...
        <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur">
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-1">{{ __('hotel.bookings.show_subheading') }}</p>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('hotel.bookings.index') }}"
                       class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-100">
                        ...
                        {{ __('hotel.bookings.actions.back') }}
                    </a>
                </div>
            </div>
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div class="space-y-4">
                    <div>
                        <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.bookings.table.user') }}</label>
                    ...
                    <div>
                        <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.bookings.table.hotel') }}</label>
                        <p class="mt-1 font-semibold text-slate-900">{{ app()->getLocale() === 'ar' ? $booking->hotel->name_ar : $booking->hotel->name_en }}</p>
                    </div>

                    @if($booking->room)
                        <div>
                            <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.bookings.table.room') }}</label>
                            <p class="mt-1 text-slate-700">{{ number_format($booking->room->price_per_night, 2) }} {{ __('hotel.bookings.currency') }}</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.bookings.table.check_in') }}</label>
                            <p class="mt-1 font-semibold text-slate-900">{{ $booking->check_in_date->format('Y-m-d') }}</p>
                        </div>
                        <div>
                            <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.bookings.table.check_out') }}</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.bookings.table.guests') }}</label>
                    ...
                    <div>
                        <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.bookings.table.nights') }}</label>
                        <p class="mt-1 text-slate-700">{{ $booking->nights_count }}</p>
                    </div>

                    <div>
                        <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.bookings.table.status') }}</label>
                        @php
                            $statusClasses = [
            ...
            @if($booking->notes)
                <div class="mt-6 pt-6 border-t border-slate-200">
                    <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.bookings.form.notes') }}</label>
            ...
            @if($booking->admin_notes)
                <div class="mt-4">
                    <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.bookings.form.admin_notes') }}</label>
        ...
        <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-bold text-slate-900">{{ __('hotel.payments.heading') }}</h3>
                <a href="{{ route('hotel.bookings.payments.create', $booking) }}"
                   class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700">
                    ...
                    {{ __('hotel.payments.create_title') }}
                </a>
            </div>

            <div class="grid gap-4 md:grid-cols-3 mb-6">
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.bookings.table.total_price') }}</label>
                    <p class="mt-1 text-2xl font-bold text-slate-900">{{ number_format($booking->total_price, 2) }} {{ __('hotel.bookings.currency') }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-blue-50 p-4">
                    <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.payments.total_paid') }}</label>
                    <p class="mt-1 text-2xl font-bold text-blue-600">{{ number_format($booking->total_paid, 2) }} {{ __('hotel.bookings.currency') }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-emerald-50 p-4">
                    <label class="text-xs font-medium uppercase text-slate-500">{{ __('hotel.payments.remaining_amount') }}</label>
                    <p class="mt-1 text-2xl font-bold text-emerald-600">{{ number_format($booking->remaining_amount, 2) }} {{ __('hotel.bookings.currency') }}</p>
                </div>
            </div>
