...
@section('title', __('admin.hotel_rooms.page_title'))
@section('page-title', __('admin.hotel_rooms.heading'))
@section('page-subtitle', __('admin.hotel_rooms.subheading'))
...
@section('content')
    <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-lg font-semibold text-slate-900">{{ __('admin.hotel_rooms.heading') }}</h2>
                <p class="text-sm text-slate-500">{{ __('admin.hotel_rooms.subheading') }}</p>
            </div>
                ...
                @if(request()->has('hotel_id'))
                    <a href="{{ route('admin.hotels.index') }}"
                       class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-100">
                        ...
                        {{ __('admin.hotels.heading') }}
                    </a>
                @endif
                <a href="{{ route('admin.hotel-rooms.create', request()->has('hotel_id') ? ['hotel_id' => request()->hotel_id] : []) }}"
                   class="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-md shadow-slate-400/40 transition hover:bg-slate-700">
                    ...
                    {{ __('admin.hotel_rooms.actions.create') }}
                </a>
            </div>
        </div>
        ...
        @if($hotels->count() > 0)
            <div class="mt-4">
                <form method="GET" action="{{ route('admin.hotel-rooms.index') }}" class="flex items-center gap-3">
        ...
        <div class="mt-6 overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 text-right text-sm">
                <thead class="bg-slate-50 text-xs font-medium uppercase tracking-wider text-slate-500">
                    <tr>
                        <th class="px-4 py-3">{{ __('admin.hotel_rooms.table.hotel') }}</th>
                        <th class="px-4 py-3">{{ __('admin.hotel_rooms.table.price') }}</th>
                        <th class="px-4 py-3">{{ __('admin.hotel_rooms.table.beds') }}</th>
                        <th class="px-4 py-3">{{ __('admin.hotel_rooms.table.bathrooms') }}</th>
                        <th class="px-4 py-3">{{ __('admin.hotel_rooms.table.rooms') }}</th>
                        <th class="px-4 py-3 text-center">{{ __('admin.hotel_rooms.table.status') }}</th>
                        <th class="px-4 py-3 text-center">{{ __('admin.hotel_rooms.table.actions') }}</th>
                    </tr>
                </thead>
                            ...
                            <td class="px-4 py-4 text-center">
                                @if ($room->is_active)
                                    <span class="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-600">
                                        ...
                                        {{ __('admin.hotel_rooms.badges.active') }}
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500">
                                        ...
                                        {{ __('admin.hotel_rooms.badges.inactive') }}
                                    </span>
                                @endif
                            </td>
                            <td class="px-4 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.hotel-rooms.destroy', $room) }}"
                                        ...
                                        <button type="submit"
                                            ...
                                            {{ __('admin.hotel_rooms.actions.delete') }}
                                        </button>
                                    </form>
                                </div>
                            </td>
    ...
    <!-- Clone Modal -->
    <div id="cloneModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-opacity-50">
        <div class="mx-4 w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <form id="cloneForm" method="POST">
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" 
                            class="rounded-lg bg-green-600 px-4 py-2 text-sm font-medium hover:bg-green-700">
                        استنساخ
                    </button>
                </div>
            </form>
