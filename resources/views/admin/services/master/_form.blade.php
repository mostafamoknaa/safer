    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div>
            ...
            <input type="text" name="name_ar" id="name_ar" value="{{ old('name_ar', $service->name_ar ?? '') }}"
                   class="mt-1 block w-full  h-12 rounded-xl border border-slate-700 bg-slate-50 shadow-sm
                         focus:border-slate-900 focus:ring-slate-900 sm:text-sm
                         @error('name_ar') border-rose-500 @enderror"
                   required>
            @error('name_ar')
        ...
        <div>
            ...
            <input type="text" name="name_en" id="name_en" value="{{ old('name_en', $service->name_en ?? '') }}"
                   class="mt-1 block w-full h-12 rounded-xl border border-slate-300 bg-slate-50 shadow-sm
                         focus:border-slate-900 focus:ring-slate-900 sm:text-sm
                         @error('name_ar') border-rose-500 @enderror"
                   required>
            @error('name_en')
