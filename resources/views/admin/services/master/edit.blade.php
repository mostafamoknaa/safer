...
@section('title', __('admin.services.edit_title'))
@section('page-title', __('admin.services.edit_heading'))
@section('page-subtitle', __('admin.services.edit_subheading'))

@section('content')
    <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-8 shadow-lg shadow-slate-200/60 backdrop-blur">
        <form action="{{ route('admin.master-services.update', $service) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mt-8 flex items-center justify-end gap-3 border-t border-slate-100 pt-8">
                ...
                <button type="submit"
                    ...
                    {{ __('admin.services.actions.update') }}
                </button>
            </div>
        </form>
    </div>
