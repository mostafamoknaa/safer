...
<div class="grid gap-6">
    <div class="grid gap-4 sm:grid-cols-2">
        <div class="grid gap-2">
            ...
            <input id="name_ar" name="name_ar" type="text" value="{{ old('name_ar', $hotel->name_ar ?? '') }}"
        ...
        <div class="grid gap-2">
            ...
            <input id="name_en" name="name_en" type="text" value="{{ old('name_en', $hotel->name_en ?? '') }}"
                class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                dir="ltr" required>
            @error('name_en')
                <p class="text-xs text-rose-600">{{ $message }}</p>
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        ...
        <div class="grid gap-2">
            <label for="country" class="text-sm font-medium text-slate-600">
                {{ __('admin.hotels.form.country') ?? 'الدولة' }}
            </label>
            <input id="country" name="country" type="text" value="{{ old('country', $hotel->country ?? '') }}"
                class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                dir="ltr">
            @error('country')
                <p class="text-xs text-rose-600">{{ $message }}</p>
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        ...
        <div class="grid gap-2">
            <label for="rate" class="text-sm font-medium text-slate-600">
                {{ __('admin.hotels.form.rate') ?? 'التقييم' }}
            </label>
            <input id="rate" name="rate" type="number" step="0.1" min="1" max="5"
                value="{{ old('rate', $hotel->rate ?? '2.0') }}"
                class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                dir="ltr">
            @error('rate')
                <p class="text-xs text-rose-600">{{ $message }}</p>
        ...
        <div class="bg-slate-50 rounded-xl p-6 border border-slate-200">
            ...
            <div id="servicesContainer" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                @foreach($services as $service)
                    <label
                        class="service-item flex items-center gap-3 p-3 bg-white rounded-lg border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 cursor-pointer transition-all duration-200 group"
                        data-service="{{ __('hotel.hotels.services.' . $service) }}">
                        ...
                        <span
                            class="text-sm text-slate-700 group-hover:text-indigo-700 font-medium">{{ __('hotel.hotels.services.' . $service) }}</span>
                    </label>
                @endforeach
            </div>
        </div>
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        <div class="grid gap-2">
            ...
            <input id="lat" name="lat" type="number" step="any" value="{{ old('lat', $hotel->lat ?? '') }}"
                class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                dir="ltr">
            @error('lat')
                <p class="text-xs text-rose-600">{{ $message }}</p>
        ...
        <div class="grid gap-2">
            ...
            <input id="lang" name="lang" type="number" step="any" value="{{ old('lang', $hotel->lang ?? '') }}"
                class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                dir="ltr">
            @error('lang')
                <p class="text-xs text-rose-600">{{ $message }}</p>
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        <div class="grid gap-2">
            ...
            <textarea id="address_ar" name="address_ar" rows="2"
                class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                dir="rtl" required>{{ old('address_ar', $hotel->address_ar ?? '') }}</textarea>
            @error('address_ar')
                <p class="text-xs text-rose-600">{{ $message }}</p>
            @enderror
        </div>
        <div class="grid gap-2">
            ...
            <textarea id="address_en" name="address_en" rows="2"
                class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                dir="ltr" required>{{ old('address_en', $hotel->address_en ?? '') }}</textarea>
            @error('address_en')
                <p class="text-xs text-rose-600">{{ $message }}</p>
            @enderror
        </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                @foreach($hotel->media as $media)
                    <div class="relative group">
                        ...
                        @else
                            <video src="{{ Storage::url($media->file_path) }}" class="w-full h-32 object-cover rounded-lg"
                                controls></video>
                        @endif
    ...
    <div class="flex items-center gap-3">
        <label class="inline-flex items-center gap-2 text-sm font-medium text-slate-600">
            <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $hotel->is_active ?? true))
