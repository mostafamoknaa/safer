...
@section('content')
    <div class="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-lg shadow-slate-200/60 backdrop-blur">
        ...
        {{-- Tabs for filtering --}}
        <div class="mt-6 flex flex-wrap gap-2 border-b border-slate-100 pb-4">
            @php
            ...
            @foreach($tabs as $type => $label)
                <a href="{{ route('admin.hotels.index', ['type' => $type]) }}"
                   class="inline-flex items-center rounded-full px-4 py-2 text-sm font-semibold transition {{ $currentType == $type ? 'bg-slate-900 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                    {{ $label }}
                </a>
            @endforeach
        </div>
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.ical.index', $hotel) }}"
