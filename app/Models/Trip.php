...
class Trip extends Model
{
    ...
    {
        $allUnavailable = $this->booked_seat_numbers;
        return max(0, ($this->bus->total_seats ?? 0) - count($allUnavailable));
    }
    ...
    {
        $booked = $this->bookedSeats()->pluck('seat_number')->map(fn($s) => trim((string) $s))->toArray();
        $closed = $this->closedSeats()->pluck('seat_number')->map(fn($s) => trim((string) $s))->toArray();

        return array_unique(array_merge($booked, $closed));
    }
